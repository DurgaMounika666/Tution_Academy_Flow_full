  ...
  useEffect(() => {
    ...
    const loadConversation = async () => {
      try {
        ...
        const data = await apiClient.chat.getConversation(currentTutor.id, parentEmail);
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      const sent = await apiClient.chat.send({
        ...
        senderRole: "tutor",
        receiverId: selectedStudent.parentEmail,
        receiverName: `${selectedStudent.name}'s Parent`,
        receiverRole: "parent",
        text: newMsg.trim(),
      });
...
function ReviewsView({ reviews, students }: { reviews: Review[]; students: Student[] }) {
  ...
  useEffect(() => {
    // Load tutor's reviews for students
    const load = async () => {
      try {
        ...
        for (const s of students) {
          ...
          if (Array.isArray(data)) {
            allReviews.push(...data.filter((r: any) => r.type === "student_tutor"));
          }
        }
        setTutorReviews(allReviews);
      } catch {
        setTutorReviews([]);
      }
    };
  ...
  return (
    <div className="space-y-6 text-left">
      {/* Write Review Form */}
      <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 shadow-sm border border-slate-100 dark:border-slate-800 space-y-4">
        <h3 className="text-sm font-black uppercase tracking-wider text-slate-400">Write Review for Student</h3>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex gap-1">
              {[1, 2, 3, 4, 5].map((star) => (
                ...
                >
                  <Star className={`h-5 w-5 transition-colors ${star <= reviewRating ? "fill-amber-400 text-amber-400" : "text-slate-300 hover:text-amber-300"}`} />
                </button>
              ))}
            </div>
          </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 shadow-sm border border-slate-100 dark:border-slate-800 space-y-4">
        ...
        <div className="space-y-4">
          ...
          {reviews.length === 0 && <p className="text-xs text-slate-500">No reviews from students yet.</p>}
        </div>
      </div>

      {/* Tutor's Submitted Reviews */}
      <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 shadow-sm border border-slate-100 dark:border-slate-800 space-y-4">
        <p className="text-[10px] font-black uppercase tracking-wider text-slate-400 mb-3">Your Reviews For Students</p>
        <div className="space-y-4">
          {tutorReviews.map((r: any, idx: number) => (
            <div key={r._id || idx} className="p-4 bg-slate-50 dark:bg-slate-950 rounded-2xl border border-slate-100 dark:border-slate-850 flex items-start gap-3">
              <StudentAvatar name={r.studentName || students.find(s => s.id === r.studentId)?.name || "Student"} />
              <div className="flex-1">
                <div className="flex items-center justify-between">
          ...
          {tutorReviews.length === 0 && <p className="text-xs text-slate-500">You haven't submitted any reviews yet.</p>}
        </div>
      </div>
    </div>
  );
